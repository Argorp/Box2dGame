...
public class LiveView extends View{
    ...
    public void draw(SpriteBatch batch) {
        ...
        if (leftLives > 3) batch.draw(texture, x + 3 * (texture.getWidth() + livePadding), y, width, height);
        if (leftLives > 4) batch.draw(texture, x + 4 * (texture.getWidth() + livePadding), y, width, height);
    }
